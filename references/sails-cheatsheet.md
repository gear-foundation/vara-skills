...
- In `0.10.2`, `msg::source()` and `exec::program_id()` require `use sails_rs::gstd::{msg, exec};`. The `prelude::*` does not re-export them. Missing this import causes `unresolved module` errors when writing guards or delayed-message helpers.
