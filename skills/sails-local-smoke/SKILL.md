...
7. Exercise one command and one query to prove the integration works:
   ...
   ```
   - **vara-wallet v2 IDL compatibility:** If `--idl` calls fail with parsing errors (e.g., "Invalid token"), vara-wallet may not support the v2 IDL format generated by sails-rs 1.0.0-beta.2. Upgrade vara-wallet to a version that supports v2 IDL, or use the gclient secondary path instead.
...
- If vara-wallet cannot parse the program's v2 IDL, fall back to the gclient secondary path rather than skipping local-smoke validation.
